    ...
    <div class="flex flex-1 overflow-hidden">
        ...
        <div class="w-64 bg-white border-r shadow-inner flex flex-col shrink-0">
            ...
            <nav class="flex-1 mt-2">
                ...
                <a href="{{ route('reservations.index') }}" class="block py-3 px-6 text-gray-600 hover:bg-gray-100 hover:text-iium-green transition border-l-4 border-transparent">
                    My Reservation
                </a>
                <a href="{{ route('profile.show') }}" class="block py-3 px-6 bg-iium-green text-white font-bold border-l-4 border-iium-gold shadow">
                    User Detail
                </a>
            </nav>
                ...
                <div class="w-2/3 p-8">
                    ...
                    <form action="{{ route('profile.update') }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="name" value="{{ old('name', $user->name) }}" class="w-full bg-white border border-gray-300 p-2 rounded text-gray-800 focus:border-[#007A5E] focus:ring-1 focus:ring-[#007A5E]">
                        </div>

                        <div class="grid grid-cols-2 gap-4 mb-4">
                            <div>
                                <label class="block text-gray-500 text-xs font-bold uppercase mb-1">Phone Number</label>
                                <input type="text" name="phone" value="{{ old('phone', $user->phone ?? '') }}" class="w-full bg-white border border-gray-300 p-2 rounded text-gray-800 focus:border-[#007A5E] focus:ring-1 focus:ring-[#007A5E]">
                            </div>
                            <div>
                                <label class="block text-gray-500 text-xs font-bold uppercase mb-1">Email</label>
                                <input type="email" name="email" value="{{ old('email', $user->email) }}" class="w-full bg-white border border-gray-300 p-2 rounded text-gray-800 focus:border-[#007A5E] focus:ring-1 focus:ring-[#007A5E]">
                            </div>
                        </div>
                        
                        <hr class="my-6 border-gray-200">
                        <p class="text-xs text-gray-400 mb-4 font-bold uppercase">Change Password (Optional)</p>

                        <div class="mb-4">
                            <label class="block text-gray-500 text-xs font-bold uppercase mb-1">New Password</label>
                            <input type="password" name="password" class="w-full bg-white border border-gray-300 p-2 rounded text-gray-800 focus:border-[#007A5E] focus:ring-1 focus:ring-[#007A5E]">
                        </div>

                        <div class="mb-6">
                            <label class="block text-gray-500 text-xs font-bold uppercase mb-1">Confirm Password</label>
                            <input type="password" name="password_confirmation" class="w-full bg-white border border-gray-300 p-2 rounded text-gray-800 focus:border-[#007A5E] focus:ring-1 focus:ring-[#007A5E]">
                        </div>

                        <div class="text-right">
                            <button type="submit" class="bg-iium-green text-white font-bold py-2 px-6 rounded text-sm hover:bg-[#005f49] transition shadow-md">
                                Update Details
                            </button>
                        </div>
                    </form>
